...
def find_unique_items_advanced(json_data, similarity_threshold=0.85, strict_mode=False):
    ...
    for i, item in enumerate(json_data):
        ...
        for j, other_item in enumerate(json_data[i + 1:], i + 1):
            ...
            if similarity_score >= similarity_threshold:
                # Strict mode bo'lsa, yumshatilgan tekshiruvlar
                if strict_mode:
                    # Faqat juda farqli matnlarni rad etish
                    if detailed_scores['text'] < 0.7:  # 0.9 dan 0.7 ga tushirdik
                        continue

                    # Audio ID bir xil bo'lsa, albatta takroriy
                    if detailed_scores['audio_name'] == 1.0:
                        similar_items.append(other_item)
                        similar_indices.append(j)
                        continue

                # Oddiy holat - similarity_threshold dan yuqori bo'lsa qo'shish
                similar_items.append(other_item)
                similar_indices.append(j)
...
def main():
    st.set_page_config(
        page_title="Noyob Dataset Yig'uvchi",
        page_icon="🎯",
        layout="wide"
    )

    st.title("🎯Noyob Dataset Yig'uvchi")
    ...
    if st.session_state.selected_folders:
        ...
        with col1:
            similarity_threshold = st.slider(
                ...
                min_value=60, max_value=95, value=85, step=5,
        ...
        with col2:
            strict_mode = st.checkbox(
                "🔒 Qattiq rejim",
                value=False,  # Default False qilamiz
                help="Juda o'xshash elementlar uchun qo'shimcha tekshiruvlar"
            )
